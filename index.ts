...
let _getProtocols: (() => Promise<string[] | null>) | null = null;
...
export class WorkerWsController {
    ...
    constructor(private workerCtx: Window, private path: string, private options: WorkerWsCtrlOptions = {}) {
        if (path.startsWith('/')) {
    ...
    start(apiUrl: string, getProtocols?: () => Promise<string[] | null>) {
        this.apiUrl = apiUrl;
    ...
    subscribe(onEvent: (ev: string) => void, ping: (pong: () => void) => void, onConnect: () => void, onClose: () => void) {
        const s = { onEvent, ping, onConnect, onClose };
        subscribers.add(s);
    ...
    private async startWs() {
        if (ws) return;
        try {
            const protocols = await _getProtocols?.();
